...
typedef eRadioStatus_t (*parseFunction)(char *respToParse);

eRadioStatus_t bg95_sendAtCmd(const char *txData, char *rxData, uint32_t timeout, uint32_t txDataSize);
eRadioStatus_t bg95_serialTx(const char *txData, char *rxData, uint32_t timeout, uint32_t txDataSize, uint8_t mqttCmd);
eRadioStatus_t bg95_serialRX(char *response, uint32_t respTimeout, uint8_t isMqttCmd);
eRadioStatus_t bg95_parseResponse(char *respToParse);
eRadioStatus_t parse_cpin(char *respToParse);
eRadioStatus_t parse_creg(char *respToParse);
eRadioStatus_t parse_cgreg(char *respToParse);
eRadioStatus_t parse_cereg(char *respToParse);
eRadioStatus_t parse_csq(char *respToParse);
eRadioStatus_t parse_cme(char *respToParse);
eRadioStatus_t parser_serialNumber(char *respToParse);
eRadioStatus_t parse_mqttOpen(char *respToParse);
eRadioStatus_t parse_mqttConn(char *respToParse);
eRadioStatus_t parse_mqttDisc(char *respToParse);
eRadioStatus_t parse_mqttSub(char *respToParse);
eRadioStatus_t parse_mqttUns(char *respToParse);
eRadioStatus_t parse_mqttPubex(char *respToParse);
...
void bg95_turnOff(void)
{
	...
	if(bg95_sendAtCmd(AT_QPOWD, rxBuf, CONFIG_TIMEOUT, sizeof(AT_QPOWD)) != radio_ok)
	{
		while(1);
	}
}
...
{
	if(bg95_sendAtCmd(AT_GSN, serialNumBuf, CONFIG_TIMEOUT, sizeof(AT_GSN)) == radio_error)
	{
		breakpoint();
		return radio_error;
	}
...
//
//	if(bg95_sendAtCmd(AT_ATE0, rxBuf, CONFIG_TIMEOUT, sizeof(AT_ATE0)) != radio_ok)
//	{
//		breakpoint();
//		return radio_error;
//	}

	if(bg95_sendAtCmd(AT_CPIN, rxBuf, CPIN_TIMEOUT, sizeof(AT_CPIN)) != radio_ok)
	{
		breakpoint();
		return radio_error;
	}

	if(bg95_sendAtCmd(AT_QCFG_NWSCANMODE, rxBuf, CONFIG_TIMEOUT, sizeof(AT_QCFG_NWSCANMODE)) != radio_ok)
	{
		breakpoint();
		return radio_error;
	}

	if(bg95_sendAtCmd(AT_QCFG_NWSCANSEQ, rxBuf, CONFIG_TIMEOUT, sizeof(AT_QCFG_NWSCANSEQ)) != radio_ok)
	{
		breakpoint();
		return radio_error;
	}

	if(bg95_sendAtCmd(AT_QCFG_IOTOPMODE, rxBuf, CONFIG_TIMEOUT, sizeof(AT_QCFG_IOTOPMODE)) != radio_ok)
	{
		breakpoint();
		return radio_error;
	}

	if(bg95_sendAtCmd(AT_QCFG_BAND, rxBuf, CONFIG_TIMEOUT, sizeof(AT_QCFG_BAND)) != radio_ok)
	{
		breakpoint();
		return radio_error;
	}

	if(bg95_sendAtCmd(AT_QICSGP, rxBuf, CONFIG_TIMEOUT, sizeof(AT_QICSGP)) != radio_ok)
	{
		breakpoint();
		return radio_error;
	}

	if(bg95_sendAtCmd(AT_CFUN0, rxBuf, CFUN_TIMEOUT, sizeof(AT_CFUN0)) != radio_ok)
	{
		breakpoint();
		return radio_error;
	}
	...
	if(bg95_sendAtCmd(AT_CFUN1, rxBuf, CFUN_TIMEOUT, sizeof(AT_CFUN1)) != radio_ok)
	{
		breakpoint();
		return radio_error;
	}
...
eRadioStatus_t bg95_checkSignal(void)
{
	...
	if(bg95_sendAtCmd(AT_QNWINFO, rxBuf, CONFIG_TIMEOUT, sizeof(AT_QNWINFO)) != radio_ok)
	{
		breakpoint();
		return radio_error;
	}
...
//	if(bg95_sendAtCmd(AT_CSQ, rxBuf, CONFIG_TIMEOUT, sizeof(AT_CSQ)) == radio_error)
//	{
//		breakpoint();
//		return radio_error;
//	}
...
eRadioStatus_t bg95_connect(char *mcu_uid)
{
	...
	eRadioStatus_t 	creg	= radio_error,
					cgreg	= radio_error,
					cereg	= radio_error;
	...
	if(bg95_sendAtCmd(AT_CGATT1, rxBuf, CGATT_TIMEOUT, sizeof(AT_CGATT1)) != radio_ok)
	{
		breakpoint();
		return radio_error;
	}

	if(bg95_sendAtCmd(AT_QMTOPEN, rxBuf, (MQTT_TIMEOUT * 2), sizeof(AT_QMTOPEN)) != radio_ok)
	{
		breakpoint();
		return radio_error;
	}

	if(bg95_sendAtCmd(AT_QMTCONN, rxBuf, MQTT_TIMEOUT, sizeof(AT_QMTCONN)) != radio_ok)
	{
		breakpoint();
		return radio_error;
	}
...
eRadioStatus_t bg95_publish(const char *msg)
{
	...
	if(bg95_sendAtCmd(AT_QMTPUBEX, rxBuf, MQTT_TIMEOUT, sizeof(AT_QMTPUBEX)) != radio_ok)
	{
		breakpoint();
		return radio_error;
	}
...
eRadioStatus_t bg95_disconnect(void)
{
	...
	if(bg95_sendAtCmd(AT_QMTDISC, rxBuf, MQTT_TIMEOUT, sizeof(AT_QMTDISC)) != radio_ok)
	{
		breakpoint();
		return radio_error;
	}

	if(bg95_sendAtCmd(AT_QIDEACT, rxBuf, QIDEACT_TIMEOUT, sizeof(AT_QIDEACT)) != radio_ok)
	{
		breakpoint();
		return radio_error;
	}

	if(bg95_sendAtCmd(AT_CGATT0, rxBuf, CGATT_TIMEOUT, sizeof(AT_CGATT0)) != radio_ok)
	{
		breakpoint();
		return radio_error;
	}
...
eRadioStatus_t bg95_sendAtCmd(const char *txData, char *rxData, uint32_t timeout, uint32_t txDataSize)
{
	...
	if(bg95_serialTx(txData, rxData, timeout, txDataSize, flagMqttCmd) == radio_timeout)	//bg95_timeout)
	{
		breakpoint();
		return radio_timeout;	//bg95_timeout;
	}
...
eRadioStatus_t bg95_parseResponse(char *respToParse)
{
	...
	if(c0 == '+')
	{
		...
		{
			breakpoint();
			return radio_cmdNotRegistred;	//bg95_cmdNotRegistred;
		}
	}
	else
	{
...
eRadioStatus_t parse_csq(char *respToParse)
{
	...
	{
		return radio_noSignal;	//csq_noSignal;
	}
	else if((respToParse[5] <= '1') && (respToParse[6] <= '7'))
	{
		return radio_poorSignal;	//csq_poorSignal;
	}
	else if((respToParse[5] >= '3') && (respToParse[6] >= '1'))
	{
		return radio_greatSignal;	//csq_greatSignal;
	}
	else
	{
		return radio_goodSignal;	//csq_goodSignal;
	}
	breakpoint();
	return radio_error;
}
